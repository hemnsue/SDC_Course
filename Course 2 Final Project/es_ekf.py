...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    err=k_gain.dot(y_k-p_check)
    # 3.3 Correct predicted state
    p_hat=p_check+err[0:3]
    v_hat=v_check+err[3:6]

    q_hat=Quaternion(euler=err[6:9]).quat_mult_left(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    #rotaion matrix between the navigation and sensor frame
    ...
    #p_cov[k]=F_k.dot(p_cov[k-1]).dot[F_k].T+l_jac.dot(Q_k).dot(l_jac.T)
    p_cov[k] = F_k.dot(p_cov[k-1]).dot(F_k.T) + l_jac.dot(Q_k).dot(l_jac.T)
    '''# 1. Update state with IMU inputs (Prediction Step)
    delta_t = imu_f.t[k] - imu_f.t[k - 1]

    # Rotation matrix between navigation and sensor frame
    C_ns = Quaternion(*q_est[k-1]).to_mat()

    # Motion model
    # Position update using previous velocity and acceleration
    p_est[k] = p_est[k-1] + delta_t * v_est[k-1] + 0.5 * (delta_t**2) * (C_ns @ imu_f.data[k-1] + g)

    # Velocity update using previous velocity and IMU data
    v_est[k] = v_est[k-1] + delta_t * (C_ns @ imu_f.data[k-1] + g)

    # Quaternion update (rotation using IMU rotational velocity)
    q_est[k] = Quaternion(axis_angle=imu_w.data[k-1] * delta_t).quat_mult_right(q_est[k-1])
    ...
    if lidar_i<lidar.t.shape[0]:
        if lidar.t[lidar_i]==imu_f.t[k-1]:
            ...
            lidar_i += 1
    '''# 3. Check availability of GNSS and LIDAR measurements (Correction Step)
    if gnss_i < gnss.t.shape[0]:
        if np.isclose(gnss.t[gnss_i], imu_f.t[k], atol=1):  # Adjust to handle small timing differences
            p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_gnss, p_cov[k], gnss.data[gnss_i].T, p_est[k], v_est[k], q_est[k])
            gnss_i += 1

    if lidar_i < lidar.t.shape[0]:
        if np.isclose(lidar.t[lidar_i], imu_f.t[k], atol=1):  # Adjust to handle small timing differences
            p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_lidar, p_cov[k], lidar.data[lidar_i].T, p_est[k], v_est[k], q_est[k])
            lidar_i += 1'''
